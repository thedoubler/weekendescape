...
export interface Deal {
  ...
  co2Kg?: number | null;
  // Meet-up mode only: one FULL deal per home airport, everyone landing on
  // the same weekend. When present, `price` on the parent is the TOTAL and
  // the parent's own itinerary fields mirror the cheapest row's flight; the
  // expanded panel renders every row's legs in full. Rows never nest further
  // (a row's own `meetup` is absent).
  meetup?: Deal[];
